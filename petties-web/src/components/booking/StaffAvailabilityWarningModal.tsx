...
import type { StaffAvailabilityCheckResponse } from '../../types/booking';
import { STAFF_SPECIALTY_LABELS } from '../../types/booking';

export type ConfirmOption = 'partial' | 'remove' | 'cancel';

interface StaffAvailabilityWarningModalProps {
    isOpen: boolean;
    availability: StaffAvailabilityCheckResponse;
    onClose: () => void;
...
export const StaffAvailabilityWarningModal: React.FC<StaffAvailabilityWarningModalProps> = ({
    isOpen,
    availability,
    onClose,
    onConfirm,
    ...
    const unavailableServices = availability.services.filter((s) => !s.hasAvailableStaff);
    const availableServices = availability.services.filter((s) => s.hasAvailableStaff);
            ...
            <div className="w-full max-w-2xl bg-white border-4 border-stone-900 shadow-brutal max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-4 border-b-4 border-stone-900 bg-amber-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="font-bold text-lg">Cảnh báo thiếu nhân viên</h2>
                            <p className="text-sm text-stone-600">
                                {unavailableServices.length} dịch vụ chưa có nhân viên phù hợp
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        {availability.services.map((service) => (
                            <div
                                ...
                                className={`p-3 border-2 border-stone-900 flex items-center justify-between ${
                                    service.hasAvailableStaff ? 'bg-green-50' : 'bg-red-50'
                                }`}
                            >
                                <div className="flex items-center gap-3">
                                    {service.hasAvailableStaff ? (
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-stone-600">
                                        Xác nhận booking và gán nhân viên thủ công cho các dịch vụ
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                />
                                <div>
                                    <p className="font-bold">Hủy để thêm lịch nhân viên trước</p>
                                    <p className="text-sm text-stone-600">
                                        Đóng modal và thêm ca làm việc cho nhân viên trước, sau đó
