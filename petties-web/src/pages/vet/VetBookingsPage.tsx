/**
 * VetBookingsPage - Page for Vet to view their assigned bookings
...
export const VetBookingsPage = () => {
    ...
    // Handle navigation from VetSchedulePage with focusBookingId
    ...
    const fetchBookings = useCallback(async () => {
        ...
        try {
            ...
            const response = await getBookingsByVet(user.userId, status, page, pageSize)
    ...
    useSseNotification({
        onBookingUpdate: (data) => {
            console.log('[VetBookingsPage] Booking update received:', data)
                    ...
                    {filteredBookings.map(booking => {
                        ...
                        return (
                            ...
                            >
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                            ...
                                            <div className="flex items-center gap-2 text-stone-600">
                                                ...
                                                <span>{(() => {
                                                    ...
                                                    // Calculate end time from vet's services
                                                    const vetServices = booking.services?.filter(
                                                        svc => svc.assignedVetId === user?.userId
                                                    ) || [];

                                                    if (vetServices.length > 0) {
                                                        // Find the latest end time among vet's services
                                                        const endTimes = vetServices
                                ...
                                {/* Services - Only show vet's assigned */}
                                {(() => {
                                    // Filter to only show services assigned to this vet
                                    const vetServices = booking.services?.filter(
                                        svc => svc.assignedVetId === user?.userId
                                    ) || [];
                                    return vetServices.length > 0 && (
                                        <div className="mt-3 pt-3 border-t border-stone-100">
                                            <div className="flex flex-wrap gap-2">
                                                {vetServices.map((svc, idx) => (
                        ...
                        {detailLoading ? (
                            ...
                            <>
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    {/* Services - ONLY show services assigned to this vet */}
                                    {(() => {
                                        ...
                                        return myServices.length > 0 && (
                                            <>
                                                ...
                                                {/* Total Price for vet's services */}
