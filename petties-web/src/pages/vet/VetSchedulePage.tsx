...
export const VetSchedulePage = () => {
    ...
    const fetchShifts = useCallback(async () => {
        ...
        try {
            ...
            // Sử dụng API lấy lịch riêng của bác sĩ
            const data = await vetShiftService.getMyShifts(startDate, endDate)
            setShifts(data)
        } catch (err: any) {
    ...
    useEffect(() => {
        ...
        const fetchDayShifts = async () => {
            ...
            const detailPromises = myDayShifts.map(s => vetShiftService.getShiftDetail(s.shiftId))
            try {
    ...
    // Block/Unblock slot đã bị loại bỏ - Vet không có quyền block slot
                ...
                <header className="flex justify-between items-center bg-white p-4 rounded-2xl border-2 border-stone-900 shadow-[4px_4px_0px_0px_rgba(28,25,23,0.1)]">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden md:flex flex-col items-end text-right">
                            <p className="text-[9px] font-black text-stone-400 uppercase tracking-[0.2em]">Bác sĩ phụ trách</p>
                            ...
                            <div className="flex flex-col items-center mb-8">
                                <div className="relative mb-4 group">
                                    ...
                                    {selectedShift.vetAvatar ? (
                                        <img src={selectedShift.vetAvatar} alt="avt" className="w-24 h-24 rounded-full border-4 border-stone-900 shadow-[6px_6px_0_0_#fbbf24] object-cover relative z-10 transition-transform group-hover:scale-105" />
                                    ) : (
                                ...
                                <h3 className="font-black text-stone-900 text-lg uppercase tracking-wider">{selectedShift.vetName}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="px-3 py-0.5 bg-stone-100 text-stone-500 border border-stone-200 rounded-full text-[10px] font-black uppercase tracking-tighter">Bác sĩ chuyên khoa</span>
                                </div>
                            </div>
