...
**Status:** Canonical analysis synced with current codebase
...
- `prompt_versions` has been removed from the active schema.
...
|---|---|---|
| `petties_knowledge_base` | Text RAG for uploaded knowledge documents | single text vector |
| `petties_case_memory_v2` | Retrieval over confirmed EMR cases | named vectors: `text`, `image` |
| `petties_kb_images` | Hybrid retrieval over images extracted from knowledge PDFs | named vectors: `text`, `image` |
