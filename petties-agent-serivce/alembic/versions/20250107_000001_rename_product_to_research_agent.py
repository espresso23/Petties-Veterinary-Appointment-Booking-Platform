...
def upgrade() -> None:
    ...
    # Step 2: Update existing 'product' records to 'research'
    # IMPORTANT: Cast to ::text to avoid enum validation error when 'product' doesn't exist
    # PostgreSQL validates enum literals at parse time, not execution time
    # So we must use text comparison to safely handle missing enum values
    op.execute("""
        UPDATE agents
        SET agent_type = 'research'
        WHERE agent_type::text = 'product'
    """)
...
def downgrade() -> None:
    ...
    # Step 2: Revert agent_type back to 'product' (using ::text cast for safety)
    op.execute("""
        UPDATE agents
        SET agent_type = 'product'
        WHERE agent_type::text = 'research'
    """)
