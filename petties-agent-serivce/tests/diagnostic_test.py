...
async def _run_test_tool():
    ...
    set_tool_runtime_context(
        ToolRuntimeContext(
            user_id="test", role="CLINIC_OWNER", auth_token="test", clinic_id="test"
        )
    )
    ...
    with patch(
        "app.core.tools.mcp_tools.clinic_tools.BackendClient", return_value=client
    ):
        try:
